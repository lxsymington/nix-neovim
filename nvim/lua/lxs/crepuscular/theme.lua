	...
	__index = function(_, key)
		...
		return lush(function(injected_functions)
			...
			return {
				...
				BoldItalicUnderdouble({ gui = 'nocombine,bold,italic,underdouble' }),
				...
				BoldUnderdouble({ gui = 'nocombine,bold,underdouble' }),
				...
				ItalicUnderdouble({ gui = 'nocombine,italic,underdouble' }),
				...
				NotifyDEBUGBody({ fg = Normal.fg.mix(standard.blue, 25) }),
				NotifyERRORBody({ fg = Normal.fg.mix(standard.red, 25) }),
				NotifyINFOBody({ fg = Normal.fg.mix(standard.purple, 25) }),
				NotifyTRACEBody({ fg = Normal.fg.mix(standard.orange, 25) }),
				NotifyWARNBody({ fg = Normal.fg.mix(standard.yellow, 25) }),
				...
				EyelinerPrimary({ fg = standard.cyan, gui = BoldUnderdash.gui, sp = bright.orange }),
				EyelinerSecondary({ fg = dim.cyan, gui = Underdot.gui, sp = dim.orange }),

				DapBreakpoint({
					bg = Normal.bg.mix(dim.green, 5),
					fg = standard.red,
					gui = BoldUnderdouble.gui,
					sp = standard.green,
				}),
				...
				DapBreakpointCondition({
					bg = Normal.bg.mix(dim.yellow, 5),
					fg = standard.purple,
					gui = Underdouble.gui,
					sp = standard.yellow,
				}),
				DapBreakpointConditionLine({
					bg = DapBreakpointCondition.bg,
					gui = BoldUnderdot.gui,
					sp = DapBreakpointCondition.sp,
				}),
				DapBreakpointConditionNumber({
					bg = DapBreakpointCondition.bg,
					fg = DapBreakpointCondition.fg,
				}),
				DapLogPoint({
					bg = Normal.bg.mix(bright.orange, 5),
					fg = standard.blue,
					gui = Underdot.gui,
					sp = dim.orange,
				}),
				...
				DapStopped({ bg = Normal.bg.mix(dim.purple, 5), fg = standard.yellow, gui = Bold.gui }),
				DapStoppedLine({ bg = DapStopped.bg, gui = BoldUnderdot.gui, sp = standard.yellow }),
				DapStoppedNumber({ bg = DapStopped.bg, fg = DapStopped.fg }),
				DapBreakpointRejected({
					bg = Normal.bg.mix(dim.red, 5),
					fg = standard.grey,
					gui = BoldUndercurl.gui,
					sp = standard.red,
				}),
				DapBreakpointRejectedLine({
					bg = DapBreakpointRejected.bg,
					gui = BoldUndercurl.gui,
					sp = DapBreakpointRejected.sp,
				}),
				DapBreakpointRejectedNumber({
					bg = DapBreakpointRejected.bg,
					fg = DapBreakpointRejected.fg,
				}),
			}
		end)
	end,
