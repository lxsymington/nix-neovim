...
local bright = colours.light.bright
local dim = colours.light.dim
local standard = colours.light.standard
...
local theme = lush(function(injected_functions)
	...
	return {
		...
		Normal({ bg = standard.white, fg = standard.black }), -- Normal text
		...
		MsgArea({ fg = dim.black }), -- Area for messages and cmdline
		...
		FloatBorder({ fg = dim.grey.mix(Normal.bg, 75) }), -- Border of floating windows.
		...
		PmenuExtra({ fg = dim.black }), -- Popup menu: Normal item "extra text"
		...
		Search({ bg = dim.black }), -- Last search pattern highlighting (see 'hlsearch'). Also used for similar items that need to stand out.
		...
		String({ fg = dim.black }), --   A string constant: "this is a string"
		...
		Special({ fg = bright.black, gui = Underdot.gui, sp = standard.purple }), -- (*) Any special symbol
		...
		Delimiter({ fg = standard.yellow.mix(dim.black, 25) }), --   Character that needs attention
		...
		CmpItemAbbr({ fg = standard.black, gui = NoCombine.gui }),
		...
		NeogitHunkHeader({ fg = bright.purple, bg = dim.black, gui = NoCombine.gui }),
		...
		NeotestFocused({ bg = bright.black, gui = NoCombine.gui }),
